...
fn map_to_graph(log_map: HashMap<String, Vec<HashMap<String, String>>>, keys_to_print: &Vec<String>) -> Graph<String, i32> {
...
fn parse_log(content: String, pattern: String, key_del: String, data_del: String, group_by: &String, label_keys: &Vec<String>) {
    ...
    let log_graph = map_to_graph(log_map, label_keys);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    parse_log(content, opts.pattern, opts.keys_delimiter,  opts.params_delimiter, &opts.group_by, &opts.keys_to_print);
    Ok(())
}